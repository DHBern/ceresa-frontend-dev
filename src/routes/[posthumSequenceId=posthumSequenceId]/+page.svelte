<script lang="ts">
	...
	import type { TSeqPosthumKeys } from '$lib/types/TSequences.js';
...
<!-- Series -->
{#snippet keyList(keys: (TSeqPosthumKeys | 'separator')[])}
		...
		{#each keys as key (key)}
			...
			{:else}
				<a
					...
					href={resolve(`/${key as string}`)}>{data.sequences[key as TSeqPosthumKeys].name}</a
				>
			{/if}
		{/each}
	</div>
...
<div class="px-10">
	...
	<div class="grid h-full grid-cols-1 gap-5 md:grid-cols-3 lg:grid-cols-3">
		{#each seq?.docs as TPosthumKeys[] as docKey (docKey)}
			{@const item = data.docs[docKey as TPosthumKeys]}
			<a
